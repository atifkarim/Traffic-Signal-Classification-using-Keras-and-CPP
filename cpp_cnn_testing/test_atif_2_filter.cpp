...
std::vector<std::string> vecOfStr;
    ...
    cout<<endl;
    cout<<endl<<"\n\nrow of dense kernel: "<<numbers.size()<<"\n\ncolumn of dense kernel:"<<numbers[0].size();

    return numbers;

}

Matrix convolution_kernal (){
...
std::vector<std::string> vecOfStr;
    ...
    bool result = getFileContent("/home/atif/image_classification_c++/all_text_file_of_kernal/conv_kernel_traffic_cl_9_ep_500_gray_for_cpp.txt", vecOfStr);


    Matrix conv_kernal_1(Matrix(vecOfStr.size(),Array()));
	//std::vector<std::vector<long double> > numbers;
...
//            cout<<ss<<endl;
	...
	for(int i= 0; i<conv_kernal_1.size();i++)
	{
        for(int j= 0; j<conv_kernal_1[0].size();j++)
        {
    ...
    cout<<endl;
    cout<<endl<<"\n\nRow of convolution kernal_1 is: "<<conv_kernal_1.size()<<"\n\nColumn of convolution kernal_1 is:"<<conv_kernal_1[0].size()<<endl;
...
//    cout<<"\nResized nomalized image's matrix: \n"<<new_mat<<endl; //48*48
    ...
    cout<<"\nhelooooooooo: "<<image_1.size()<<endl;
...
int main()
{
    ...
    Matrix convolution_filter_1 = convolution_kernal();

    Matrix preprocessed_image = loadImage("/home/atif/image_classification_c++/stop.ppm");

    //Matrix convImage = applyFilter(preprocessed_image, conv_kernal);

    Matrix convImage = applyFilter(preprocessed_image, convolution_filter_1);
