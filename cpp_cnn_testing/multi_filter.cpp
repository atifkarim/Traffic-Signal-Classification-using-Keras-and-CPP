        ...
        for (i=0 ; i<newImageHeight ; i++)
        {
            for (j=0 ; j<newImageWidth ; j++)
            {
                for (h=i ; h<i+filterHeight ; h++)
                {
                    ...
                    cout<<"d: "<<d<<endl;
//                    d=d+(0.121489);
                    double rel= d+p; //here this value is conv_kernel_bias
...
int main()
{
    ...
    Image preprocessed_image = loadImage("/home/atif/image_classification_c++/stop.ppm");
...
//    Matrix convImage = applyFilter(preprocessed_image, conv_kernal);
    ...
    Image convImage = applyFilter(preprocessed_image, convolution_filter_1);
...
//
    ...
    frame.at<double>(q,w)=(double)convImage[0][q][w];
